...
const Profile = () => {
  ...
  const UserProfileImage = ({ items }) => {
    const image_src = items.profile_image.rawvalue ? Url + JSON.parse(items.profile_image.rawvalue)["imagefile"] : "./assets/images/article-no-image.png";
    return (
      <>
        <div className="edit-image">
          <IonIcon icon={camera} />
          <input type="file" name="profile_image" onChange={(event) => onFileChange(event)} />
        </div>
  ...
  const UserCover = ({ items }) => {
    const image_src = items.cover_image.rawvalue ? Url + JSON.parse(items.cover_image.rawvalue)["imagefile"] : "./assets/images/article-no-image.png";
    return (
      <>
        <div className="edit-image">
          <IonIcon icon={camera} />
          <input type="file" name="cover_image" onChange={(event) => onFileChange(event)} />
        </div>
        <img src={image_src} alt={items.username} />
      </>
    );
      ...
      <OwlCarousel className="game-slider owl-theme" {...options}>
        {Object.keys(items).length > 0 &&
          items.map((game, index) => {
            ...
            const image_src = game.image ? Url + game.image : "./assets/images/article-no-image.png";
  ...
  const submitForm = (formdata) => {
    const getLocalstorage = async () => {
      ...
      if (authGlobalContext.db) {
        const UserSession = await authGlobalContext.db.get("UserSession");
        if (UserSession) {
          fetch(Url + "index.php?option=com_ajax&group=system&plugin=ajax&format=json&type=profile&uid=" + UserSession.id + "&session=" + UserSession.session, {
            method: "POST",
            body: formdata,
            headers: {},
          })
        ...
        <div className="profile-wrapper mt-2">
          <form onSubmit={handleSubmit(onFormSubmit)}>
            <Controller
              ...
              render={({ field: { value, onBlur, onChange } }) => <IonInput type="text" value={value} onIonBlur={onBlur} onInput={onChange} onIonChange={onChange} placeholder="Full Name" />}
            />
            ...
            <Controller
              ...
              render={({ field: { value, onBlur, onChange } }) => <IonTextarea value={value} onIonBlur={onBlur} onInput={onChange} onIonChange={onChange} placeholder="Biography" />}
            />
              ...
              <div className="article-desc mt-2">
                ...
                <div
                  dangerouslySetInnerHTML={{
                    __html: items.biography.rawvalue ? items.biography.rawvalue : "...",
                  }}
                />
                ...
                {Object.keys(items.games).length > 0 ? <UserGames items={items.games} /> : <> No Games Added</>}
              </div>
            </div>
          </>
        )}
